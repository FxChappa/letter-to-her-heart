...
function useAmbientAudio() {
  ...
  const start = async (): Promise<boolean> => {
    ...
    const attempt = (async () => {
      ...
      master.gain.exponentialRampToValueAtTime(volume, context.currentTime + 1.6);
...
function App() {
  ...
  const hearts = useMemo(() => Array.from({ length: 34 }, (_, index) => ({
    id:index,left:`${(index*29+4)%100}%`,delay:`-${(index*.83)%14}s`,duration:`${8+(index%7)*1.2}s`,size:`${10+(index%6)*4}px`,drift:`${-22+(index%9)*6}px`,opacity:`${.12+(index%5)*.06}`,symbol:index%4===0?'♡':'♥'
  })), []);
  ...
  useEffect(() => {
    ...
    }), { threshold: .32, rootMargin: '0px 0px -18% 0px' });
  ...
  const begin = async () => {
    setEntered(true);
    await start();
    window.setTimeout(() => document.getElementById('letter')?.scrollIntoView({ behavior: 'smooth' }), 350);
  };

  const submitUnlock = (event: FormEvent) => {
    event.preventDefault();
    const accepted = ['i want a lifetime of nights with you','a lifetime of nights with you','lifetime of nights with you'];
  ...
  return <main className={entered ? 'experience experience--entered' : 'experience'}>
    ...
    <section className="hero" aria-labelledby="page-title">
      ...
      <div className="hero__content">
        <p className="kicker"><Sparkles size={14} /> Santana’s private space</p>
        ...
        <button className="begin" type="button" onClick={begin}>Begin reading <Heart size={17} fill="currentColor" /></button>
    ...
    <section id="letter" className="letter" aria-label="First letter to Santana">
      ...
      <section className={`unlock ${unlocking ? 'unlock--opening' : ''}`} data-reveal aria-labelledby="unlock-title">
        ...
        {unlocking && <div className="unlock__success"><Heart fill="currentColor" /><span>You remembered.</span></div>}
      </section>

      {unlocked && <section id="second-letter" className="second-letter" aria-label="Second letter to Santana">
        <div className="second-letter__opening" data-reveal><span>Continue if you dare</span><h2>This next letter is different.</h2><p>Still me. Just a side of me you’ve already met before.</p></div>
        {secondLetter.map((chapter,index) => <ChapterCard chapter={chapter} index={index} phase="second" key={`second-${chapter.title}`} />)}
        <footer className="ending" data-reveal><Heart size={34} fill="currentColor" /><p>Not another night.<br />A whole life with you.</p><span>— Aldane</span></footer>
      </section>}
    </section>

    {toast && <div className="heart-toast" role="status" key={toast}>
      <div className="heart-toast__burst" aria-hidden="true">{burstParticles.map(particle => <span key={particle.id} style={{ '--x':particle.x,'--y':particle.y,'--r':particle.r,'--delay':particle.delay,fontSize:particle.size } as React.CSSProperties}>{particle.symbol}</span>)}</div>
    ...
    <aside className={controlsOpen ? 'audio audio--open' : 'audio'} aria-label="Sound controls">
      {controlsOpen && <div className="audio__panel"><label htmlFor="volume">Atmosphere</label><input id="volume" type="range" min="0.1" max="0.74" step="0.01" value={volume} onChange={event => setVolume(Number(event.target.value))} /></div>}
      <button className="audio__button" type="button" onClick={() => { if (playing) stop(); else void start(); setControlsOpen(false); }} aria-label={playing ? 'Turn background sound off' : 'Turn background sound on'}>{playing ? <Volume2 size={18} /> : <VolumeX size={18} />}<span>{playing ? 'Sound on' : 'Sound off'}</span></button>
